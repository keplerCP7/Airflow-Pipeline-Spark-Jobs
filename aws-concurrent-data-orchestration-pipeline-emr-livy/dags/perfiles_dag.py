...
def create_emr(**kwargs):
    cluster_id = 'j-3VZ4AHERXM1ZQ' #emr.create_cluster(region_name=region, cluster_name='movielens_cluster', num_core_nodes=2)
    return cluster_id
...
def generate_perfil_training(**kwargs):
    ...
    session_url = emr.wait_for_idle_session(cluster_dns, headers)
    statement_response = emr.submit_statement(session_url,
                                              's3://hdata-belcorp/pyspark-files/Perfiles_Training.py')
    emr.track_statement_progress(cluster_dns, statement_response.headers)
    emr.kill_spark_session(session_url)
...
generate_training = PythonOperator(
    task_id='generate_input',
    python_callable=generate_perfil_training,
    dag=dag)
